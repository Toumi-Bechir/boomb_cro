defmodule BoombWeb.Pregame.MatchesLive do
  use BoombWeb, :live_view
  require Logger

  def mount(params, _session, socket) do
    Logger.debug("MatchesLive mount with params: #{inspect(params)}")
    ...
    socket =
      ...
      |> assign(:matches, fetch_matches(sport, filter, leagues))
  ...
  def handle_info({:update, sport}, socket) do
    if sport == socket.assigns.sport do
      socket =
        socket
        |> assign(:matches, fetch_matches(sport, socket.assigns.filter, socket.assigns.leagues))
      {:noreply, socket}
    else
      {:noreply, socket}
    end
